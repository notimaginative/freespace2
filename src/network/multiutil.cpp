...
char* get_text_address( char * text, const int max_textlen, ubyte * address )
{
...
void multi_unpack_orient_matrix(ubyte *data,matrix *m)
{
	...
	m->v.rvec.xyz.z *= (data[16] & (1<<0)) ? -1.0f : 1.0f;
	m->v.uvec.xyz.z *= (data[16] & (1<<1)) ? -1.0f : 1.0f;
	m->v.fvec.xyz.z *= (data[16] & (1<<2)) ? -1.0f : 1.0f;
	m->v.fvec.xyz.x *= (data[16] & (1<<3)) ? -1.0f : 1.0f;
...
void multi_make_fake_players(int count)
{
	...
	for(idx=0;idx<count;idx++){
		if(!MULTI_CONNECTED(Net_players[idx])){
			...
			SDL_snprintf(Net_players[idx].player->callsign, CALLSIGN_LEN, "Player %d", idx);
...
/*
hack = ((ushort)orient->v.fvec.xyz.x * 32767);
			memcpy(&hack, &orient->v.fvec.xyz.x, 4);
			bitbuffer_put( &buf, hack, 32  );
			memcpy(&hack, &orient->v.fvec.xyz.y, 4);
			bitbuffer_put( &buf, hack, 32  );
			memcpy(&hack, &orient->v.fvec.xyz.z, 4);
			bitbuffer_put( &buf, hack, 32  );

			memcpy(&hack, &orient->v.uvec.xyz.x, 4);
			bitbuffer_put( &buf, hack, 32  );
			memcpy(&hack, &orient->v.uvec.xyz.y, 4);
			bitbuffer_put( &buf, hack, 32  );
			memcpy(&hack, &orient->v.uvec.xyz.z, 4);
			bitbuffer_put( &buf, hack, 32  );

			memcpy(&hack, &orient->v.rvec.xyz.x, 4);
			bitbuffer_put( &buf, hack, 32  );
			memcpy(&hack, &orient->v.rvec.xyz.y, 4);
			bitbuffer_put( &buf, hack, 32  );
			memcpy(&hack, &orient->v.rvec.xyz.z, 4);
			bitbuffer_put( &buf, hack, 32  );*/

/*
hack = bitbuffer_get_unsigned(&buf, 32);
			memcpy(&orient->v.fvec.xyz.x, &hack, 4);
			hack = bitbuffer_get_unsigned(&buf, 32);
			memcpy(&orient->v.fvec.xyz.y, &hack, 4);
			hack = bitbuffer_get_unsigned(&buf, 32);
			memcpy(&orient->v.fvec.xyz.z, &hack, 4);

			hack = bitbuffer_get_unsigned(&buf, 32);
			memcpy(&orient->v.uvec.xyz.x, &hack, 4);
			hack = bitbuffer_get_unsigned(&buf, 32);
			memcpy(&orient->v.uvec.xyz.y, &hack, 4);
			hack = bitbuffer_get_unsigned(&buf, 32);
			memcpy(&orient->v.uvec.xyz.z, &hack, 4);

			hack = bitbuffer_get_unsigned(&buf, 32);
			memcpy(&orient->v.rvec.xyz.x, &hack, 4);
			hack = bitbuffer_get_unsigned(&buf, 32);
			memcpy(&orient->v.rvec.xyz.y, &hack, 4);
			hack = bitbuffer_get_unsigned(&buf, 32);
			memcpy(&orient->v.rvec.xyz.z, &hack, 4);*/
...
int multi_pack_unpack_orient( int write, ubyte *data, matrix *orient)
{
	...
	if ( write )	{			
		...
		if((ang.h > 3.130) && (ang.h < 3.150)){
			...
			// stuff it	
			a = fl2i(orient->v.fvec.xyz.x * D_SCALE);
			CAP(a, D_MIN_RANGE, D_MAX_RANGE);			
			bitbuffer_put( &buf, a, 16  );
			a = fl2i(orient->v.fvec.xyz.y * D_SCALE);
			CAP(a, D_MIN_RANGE, D_MAX_RANGE);			
			bitbuffer_put( &buf, a, 16  );
			a = fl2i(orient->v.fvec.xyz.z * D_SCALE);
			CAP(a, D_MIN_RANGE, D_MAX_RANGE);			
			bitbuffer_put( &buf, a, 16  );

			a = fl2i(orient->v.uvec.xyz.x * D_SCALE);
			CAP(a, D_MIN_RANGE, D_MAX_RANGE);			
			bitbuffer_put( &buf, a, 16  );
			a = fl2i(orient->v.uvec.xyz.y * D_SCALE);
			CAP(a, D_MIN_RANGE, D_MAX_RANGE);			
			bitbuffer_put( &buf, a, 16  );
			a = fl2i(orient->v.uvec.xyz.z * D_SCALE);
			CAP(a, D_MIN_RANGE, D_MAX_RANGE);			
			bitbuffer_put( &buf, a, 16  );

			a = fl2i(orient->v.rvec.xyz.x * D_SCALE);
			CAP(a, D_MIN_RANGE, D_MAX_RANGE);			
			bitbuffer_put( &buf, a, 16  );
			a = fl2i(orient->v.rvec.xyz.y * D_SCALE);
			CAP(a, D_MIN_RANGE, D_MAX_RANGE);			
			bitbuffer_put( &buf, a, 16  );
			a = fl2i(orient->v.rvec.xyz.z * D_SCALE);
			CAP(a, D_MIN_RANGE, D_MAX_RANGE);			
			bitbuffer_put( &buf, a, 16  );
		} else {
		...
		if(flag){
			...
			orient->v.fvec.xyz.x = i2fl(a) / D_SCALE;			
			a = bitbuffer_get_signed(&buf, 16);
			orient->v.fvec.xyz.y = i2fl(a) / D_SCALE;			
			a = bitbuffer_get_signed(&buf, 16);
			orient->v.fvec.xyz.z = i2fl(a) / D_SCALE;			

			a = bitbuffer_get_signed(&buf, 16);
			orient->v.uvec.xyz.x = i2fl(a) / D_SCALE;			
			a = bitbuffer_get_signed(&buf, 16);
			orient->v.uvec.xyz.y = i2fl(a) / D_SCALE;			
			a = bitbuffer_get_signed(&buf, 16);
			orient->v.uvec.xyz.z = i2fl(a) / D_SCALE;			

			a = bitbuffer_get_signed(&buf, 16);
			orient->v.rvec.xyz.x = i2fl(a) / D_SCALE;			
			a = bitbuffer_get_signed(&buf, 16);
			orient->v.rvec.xyz.y = i2fl(a) / D_SCALE;			
			a = bitbuffer_get_signed(&buf, 16);
			orient->v.rvec.xyz.z = i2fl(a) / D_SCALE;			
		} else {
...
int multi_pack_unpack_orient( int write, ubyte *data, matrix *orient)
{
	...
	if ( write )	{
		...
		//} else {
			...
			// -1 to 1
			a = fl2i(rot_axis.xyz.x*SCALE); 
			b = fl2i(rot_axis.xyz.y*SCALE);
			c = fl2i(rot_axis.xyz.z*SCALE);
			d = fl2i(theta*SCALE);
		...
		// special case		
		rot_axis.xyz.x = i2fl(a)/SCALE;
		rot_axis.xyz.y = i2fl(b)/SCALE;
		rot_axis.xyz.z = i2fl(c)/SCALE;
		theta = i2fl(d)/SCALE;
...
int multi_pack_unpack_vel( int write, ubyte *data, matrix *orient, vector *pos, physics_info *pi)
{
	...
	if ( write )	{
		// output velocity
		r = vm_vec_dot( &orient->v.rvec, &pi->vel );
		u = vm_vec_dot( &orient->v.uvec, &pi->vel );
		f = vm_vec_dot( &orient->v.fvec, &pi->vel );
		...
		vm_vec_zero(&pi->vel);
		vm_vec_scale_add2( &pi->vel, &orient->v.rvec, r );
		vm_vec_scale_add2( &pi->vel, &orient->v.uvec, u );
		vm_vec_scale_add2( &pi->vel, &orient->v.fvec, f );
...
int multi_pack_unpack_desired_vel( int write, ubyte *data, matrix *orient, vector *pos, physics_info *pi, ship_info *sip)
{
	...
	if ( write )	{
		...
		r = vm_vec_dot( &orient->v.rvec, &pi->desired_vel );
		u = vm_vec_dot( &orient->v.uvec, &pi->desired_vel );
		f = vm_vec_dot( &orient->v.fvec, &pi->desired_vel );
		...
		// Convert into world coordinates
		vm_vec_zero(&pi->vel);
		vm_vec_scale_add2( &pi->desired_vel, &orient->v.rvec, r*max_vel.xyz.x );
		vm_vec_scale_add2( &pi->desired_vel, &orient->v.uvec, u*max_vel.xyz.y );
		vm_vec_scale_add2( &pi->desired_vel, &orient->v.fvec, f*max_vel.xyz.z );
