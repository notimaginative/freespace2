...
void ai_good_secondary_time( int team, int weapon_index, int max_fire_count, char *shipname )
{
	...
	SDL_assert( i < MAX_HUGE_SECONDARY_INFO );			// we've run out of room
...
int is_preferred_weapon(int weapon_num, object *firer_objp, object *target_objp)
{
	...
	SDL_assert( firer_objp->type == OBJ_SHIP );
...
void garbage_collect_path_points()
{
	...
	for ( so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so) ) {
		...
		if (shipp->ai_index != -1) {
			...
			if ((aip->path_length > 0) && (aip->path_start > -1)) {
				SDL_assert(aip->path_start < MAX_PATH_POINTS);
				aip->path_start = pp_xlate[aip->path_start];

				SDL_assert((aip->path_cur >= 0) && (aip->path_cur < MAX_PATH_POINTS));
...
void parse_aitbl()
{
	...
	while (required_string_either("#End", "$Name:")) {
		SDL_assert( Num_ai_classes < MAX_AI_CLASSES);
...
{
	SDL_assert(is_object_stealth_ship(stealth_objp));
...
//	object *ship;
	...
	*/
	...
	SDL_assert( is_object_stealth_ship(&Objects[aip->target_objnum]) );
...
void ai_update_danger_weapon(int attacked_objnum, int weapon_objnum)
{
	...
	if (aip->danger_weapon_objnum == -1) {
		...
		SDL_assert(old_weapon_objp != NULL);
	...
	*/
	...
	if (aip->target_objnum == objnum) {
		...
		if((Game_mode & GM_MULTIPLAYER) && (aip == Player_ai) && (Player_obj->type == OBJ_OBSERVER)){
			...
			SDL_assert(objnum != Ships[aip->shipnum].objnum);	//	make sure not targeting self
		}
...
//	mprintf(("cur_r: %.1f, desired_r: %.1f\n", r_mag, radius));
	SDL_assert( (vm_vec_dotprod(&r_vec, &center_objp->orient.v.fvec) < 0.0001));
	...
	SDL_assert(mag > 0.9999 && mag < 1.0001);
#endif
	...
	SDL_assert( dot >0.9999 && dot < 1.0001);
#endif
...
float get_wing_lowest_max_speed(object *objp)
{
	...
	SDL_assert(objp->type == OBJ_SHIP);
	SDL_assert((objp->instance >= 0) && (objp->instance < MAX_OBJECTS));
...
/*		int	ignore_wingnum = -(aip->ignore_objnum + 1);
		...
		SDL_assert(ignore_wingnum < MAX_WINGS);
		SDL_assert(aip->shipnum >= 0);
...
int get_nearest_objnum(int objnum, int enemy_team_mask, int enemy_wing, float range, int max_attackers)
{
	...
	if (aip->danger_weapon_objnum >= 0) {
		...
		if (danger_weapon_objp->signature == aip->danger_weapon_signature) {
			SDL_assert(danger_weapon_objp->type == OBJ_WEAPON);
...
int num_turrets_attacking(object *turret_parent, int target_objnum) 
{
	...
	SDL_assert(turret_parent->type == OBJ_SHIP);
	SDL_assert(Objects[target_objnum].type == OBJ_SHIP);
...
int get_nearest_turret_objnum(int turret_parent_objnum, ship_subsys *turret_subsys, int enemy_team_mask, vector *tpos, vector *tvec, int current_enemy, int big_only_flag)
{
	...
	weapon_travel_dist = SDL_min(Weapon_info[tp->turret_weapon_type].lifetime * Weapon_info[tp->turret_weapon_type].max_speed, Weapon_info[tp->turret_weapon_type].weapon_range);
	...
	SDL_assert(eeo.nearest_attacker_objnum < 0 || is_target_beam_valid(turret_subsys, eeo.nearest_attacker_objnum));
...
int find_turret_enemy(ship_subsys *turret_subsys, int objnum, vector *tpos, vector *tvec, int current_enemy, float fov, int big_only_flag = 0)
{
	...
	if ( enemy_objnum >= 0 ) {
		SDL_assert( !((Objects[enemy_objnum].flags & OF_BEAM_PROTECTED) && (Weapon_info[tp->turret_weapon_type].wi_flags & WIF_BEAM)) );
...
//	if (!SDL_strncasecmp(Ships[attacker->instance].ship_name, NOX("Kami"), 4)) {
...
void ai_evade_object(object *evader, object *evaded, int priority)
{
	...
	SDL_assert(evader != NULL);
	SDL_assert(evaded != NULL);
	SDL_assert(evader->instance != -1);
	SDL_assert(Ships[evader->instance].ai_index != -1);
...
void ai_ignore_object(object *ignorer, object *ignored, int priority)
{
	...
	SDL_assert(ignorer != NULL);
	SDL_assert(ignored != NULL);
	SDL_assert(ignorer->instance != -1);
	SDL_assert(Ships[ignorer->instance].ai_index != -1);
	SDL_assert(ignorer != ignored);
...
void ai_ignore_wing(object *ignorer, int wingnum, int priority)
{
	...
	SDL_assert(ignorer->instance != -1);
	SDL_assert(Ships[ignorer->instance].ai_index != -1);
	SDL_assert((wingnum >= 0) && (wingnum < MAX_WINGS));
...
void add_path_point(vector *pos, int path_num, int path_index, int modify_index)
{
	...
	if (modify_index == -1) {
		SDL_assert(Ppfp-Path_points < MAX_PATH_POINTS-1);
		pnp = Ppfp;
		Ppfp++;
	} else {
		SDL_assert((modify_index >= 0) && (modify_index < MAX_PATH_POINTS-1));
...
void copy_xlate_model_path_points(object *objp, model_path *mp, int dir, int count, int path_num, pnode *pnp, int randomize_pnt)
{
	...
	if (dir == 1) {
		...
		SDL_assert(dir == -1);	//	direction must be up by 1 or down by 1 and it's neither!
		start_index = mp->nverts-1;
		finish_index = SDL_max(-1, mp->nverts-1-count);
	}
...
void ai_stay_still(object *still_objp, vector *view_pos)
{
	...
	SDL_assert(still_objp->type == OBJ_SHIP);
	SDL_assert((still_objp->instance >= 0) && (still_objp->instance < MAX_OBJECTS));
...
void ai_dock_with_object(object *docker, object *dockee, int priority, int dock_type, int docker_index, int dockee_index)
{
	...
	SDL_assert(dockee != NULL);
	SDL_assert(docker->instance != -1);
	SDL_assert(Ships[docker->instance].ai_index != -1);
	SDL_assert(Ships[dockee->instance].ai_index != -1);
	SDL_assert( docker_index != -1 );
	SDL_assert( dockee_index != -1 );
...
void ai_start_waypoints(object *objp, int waypoint_list_index, int wp_flags)
{
	...
	SDL_assert(waypoint_list_index < Num_waypoint_lists);
	...
	SDL_assert(aip->active_goal != AI_ACTIVE_GOAL_DYNAMIC);
}
...
void ai_do_stay_near(object *objp, object *other_objp, float dist)
{
	...
	SDL_assert(objp != other_objp);		//	Bogus!  Told to stay near self.
	SDL_assert(objp->type == OBJ_SHIP);
...
void ai_form_on_wing(object *objp, object *goal_objp)
{
	...
	SDL_assert(objp != goal_objp);		//	Bogus!  Told to form on own's wing!
...
int ai_formation_object_get_slotnum(int objnum, object *objp)
{
	...
	SDL_assert(o != END_OF_LIST(&obj_used_list));	//	Didn't find objp in list of used ships.  Impossible!
...
{	
	...
	SDL_assert((aip->path_start >= 0) && (aip->path_start < MAX_PATH_POINTS));
	...
	SDL_assert((path_num >= 0) && (path_num < pm->n_paths));
	
	SDL_assert(pnp->path_index != -1);	//	If this is -1, that means we never found the model path points
...
float maybe_recreate_path(object *objp, ai_info *aip, int force_recreate_flag)
{
	...
	SDL_assert(&Ai_info[Ships[objp->instance].ai_index] == aip);
...
void set_accel_for_docking(object *objp, ai_info *aip, float dot, float dot_to_next, float dist_to_next, float dist_to_goal, ship_info *sip)
{
	...
	if (objp->phys_info.speed < 0.0f) {
		...
		if ((aip->mode == AIM_DOCK) && (dist_to_next < 150.0f) && (aip->path_start + aip->path_length - 2 == aip->path_cur)) {
			set_accel_for_target_speed(objp, sip->max_speed * SDL_max(dist_to_next/500.0f, 1.0f));
...
float ai_path()
{
	...
	if (aip->path_start == -1) {
		...
		SDL_assert(aip->goal_objnum >= 0 && aip->goal_objnum < MAX_OBJECTS);
	...
	if ((aip->path_cur + aip->path_dir - aip->path_start < num_points) || (aip->path_cur + aip->path_dir < aip->path_start))
		...
		SDL_assert((aip->path_cur - aip->path_dir >= aip->path_start) && (aip->path_cur - aip->path_dir - aip->path_start < num_points));
		//	Cleanup for above SDL_assert() which we hit too near release. -- MK, 5/24/98.
	...
	if ((dist_to_goal < MIN_DIST_TO_WAYPOINT_GOAL) || (vm_vec_dist_quick(&Pl_objp->last_pos, &Pl_objp->pos) > 0.1f)) {
		...
		if ( (vm_vec_dist_quick(&Pl_objp->pos, &gcvp) < min_dist_to_goal) ||
			...
			if (((aip->path_cur - aip->path_start) > (num_points+1)) || (aip->path_cur < aip->path_start)) {
				SDL_assert(aip->mode != AIM_DOCK);		//	If docking, should never get this far, getting to last point handled outside ai_path()
...
void ai_waypoints()
{
	...
	float		dot, dist_to_goal;//, dot_to_next;
	ship		*shipp = &Ships[Pl_objp->instance];
	ship_info	*sip = &Ship_info[shipp->ship_info_index];
	waypoint_list	*wpl;
	ai_info	*aip;
	...
	SDL_assert(wpl->count);	// What? Is this zero? Probably wp_index never got initialized!
			...
			if (wp_index >= wpl->count) {
				if (aip->wp_flags & WPF_REPEAT) {
					...
					if ( Ships[Pl_objp->instance].wingnum != -1 ) {
						...
						// -- This is legal, just stupid. -- SDL_assert( (aip->active_goal != AI_GOAL_NONE) && (aip->active_goal != AI_ACTIVE_GOAL_DYNAMIC) );
						
						//	Clean up from above SDL_assert, just in case we ship without fixing it.  (Encountered by JimB on 2/9/98)
...
void avoid_ship()
{
	...
	if (dist < radsum)
		accelerate_ship(aip, SDL_max(away_dot, 0.2f));
	else if (dist < 2*radsum)
		accelerate_ship(aip, SDL_max(away_dot, (dist - radsum) / radsum)+0.2f);
	else
...
int maybe_resume_previous_mode(object *objp, ai_info *aip)
{
	...
	if (aip->mode == AIM_EVADE_WEAPON) {
		if (timestamp_elapsed(aip->mode_time) || (((aip->nearest_locked_object == -1) || (Objects[aip->nearest_locked_object].type != OBJ_WEAPON)) && (aip->danger_weapon_objnum == -1))) {
			SDL_assert(aip->previous_mode != AIM_EVADE_WEAPON);
			...
			if (dist > (MAX_GUARD_DIST + guard_objp->radius) * 6) {
				if ((En_objp != NULL) && (En_objp->type == OBJ_SHIP)) {
					if (vm_vec_dist_quick(&guard_objp->pos, &En_objp->pos) > (MAX_GUARD_DIST + guard_objp->radius) * 6) {
						SDL_assert(aip->previous_mode == AIM_GUARD);
						aip->mode = aip->previous_mode;
...
int ai_select_primary_weapon(object *objp, object *other_objp, int flags)
{
	...
	SDL_assert( swp->current_primary_bank != -1 );		// get Alan or Allender
...
int ai_fire_secondary_weapon(object *objp, int priority1, int priority2)
{
	...
	SDL_assert(current_bank < shipp->weapons.num_secondary_banks);
...
void set_predicted_enemy_pos(vector *predicted_enemy_pos, object *pobjp, object *eobjp, ai_info *aip)
{
	...
	weapon_speed = ai_get_weapon_speed(&shipp->weapons);
	weapon_speed = SDL_max(weapon_speed, 1.0f);		// set not less than 1
...
void ai_chase_ct()
{
	...
	SDL_assert(Ships[Pl_objp->instance].ship_info_index >= 0);
	sip = &Ship_info[Ships[Pl_objp->instance].ship_info_index];
	SDL_assert(Ships[Pl_objp->instance].ai_index >= 0);
	aip = &Ai_info[Ships[Pl_objp->instance].ai_index];
	...
	if (aip->ai_flags & AIF_ATTACK_SLOWLY) {
		if ((dist_to_enemy > 200.0f) && (dist_to_enemy < 800.0f)) {
			if ((dot_from_enemy < 0.9f) || ai_near_full_strength(Pl_objp, &Ship_info[Ships[Pl_objp->instance].ship_info_index])) {
				//nprintf(("AI", " slowly "));
				accelerate_ship(aip, SDL_max(1.0f - (dist_to_enemy-200.0f)/600.0f, 0.1f));
				return;
			}
		} else
...
int maybe_avoid_big_ship(object *objp, object *ignore_objp, ai_info *aip, vector *goal_point, float delta_time)
{
	if (timestamp_elapsed(aip->avoid_check_timestamp)) {
		...
		if ((ship_num = will_collide_with_big_ship_all(Pl_objp, ignore_objp, goal_point, &collision_point, &distance, delta_time)) != -1) {
			...
			aip->avoid_check_timestamp = timestamp(2000 + SDL_min(1000, (int) (dist * 2.0f)));	//	Delay until check again is based on distance to avoid point.
...
void ai_stealth_sweep()
{
	...
	if (aip->submode_parm0 == SM_SS_SET_GOAL) {
		...
		box_size = SDL_min(200.0f, box_size);
		box_size = SDL_max(500.0f, box_size);
...
int ai_set_attack_subsystem(object *objp, int subnum)
{
	...
	SDL_assert(attacker_shipp->ai_index >= 0);
...
void ai_set_guard_wing(object *objp, int wingnum)
{
	...
	if (Wings[wingnum].current_count < 1) {
		...
		SDL_assert((leader_objnum >= 0) && (leader_objnum < MAX_OBJECTS));
		//SDL_assert(leader_objnum != objp-Objects);	//	Don't allow ships to guard themselves.
...
void ai_set_evade_object(object *objp, object *other_objp)
{
	...
	SDL_assert(other_objnum >= 0);

	SDL_assert(other_objnum != Ships[aip->shipnum].objnum);	//	make sure not targeting self
...
void ai_set_guard_object(object *objp, object *other_objp)
{
	...
	if ((other_aip->wing != -1) && (other_aip->wing != aip->wing)) {
		...
		SDL_assert(other_objnum >= 0);	//	Hmm, bogus object and we need its position for guard_vec.
...
void update_aspect_lock_information(ai_info *aip, vector *vec_to_enemy, float dist_to_enemy, float enemy_radius)
{
	...
	if (num_weapon_types && (wip->wi_flags & WIF_HOMING_ASPECT)) {
		if (dist_to_enemy > 300.0f - SDL_min(enemy_radius, 100.0f))
...
void ai_chase_big_get_separations(object *attack_objp, object *target_objp, vector *horz_vec_to_target, float *desired_separation, float *cur_separation)
{
	...
	temp = SDL_max(pm->maxs.xyz.x, pm->maxs.xyz.y);
	r_attacker = SDL_max(-pm->mins.xyz.x, -pm->mins.xyz.y);
	r_attacker = SDL_max(temp, r_attacker);

	// get radius of target (for rotations about forward)
	pm = model_get(Ships[attack_objp->instance].modelnum);
	temp = SDL_max(pm->maxs.xyz.x, pm->maxs.xyz.y);
	r_target = SDL_max(-pm->mins.xyz.x, -pm->mins.xyz.y);
	r_target = SDL_max(temp, r_target);
...
int maybe_hack_cruiser_chase_abort()
{
	...
	if ( SDL_strcasecmp(Mission_filename, "sm3-08.fs2") == 0 ) {
		if (( SDL_strcasecmp(eshipp->ship_name, "colossus") == 0 ) || ( SDL_strcasecmp(shipp->ship_name, "colossus") == 0 )) {
			...
			//if ( SDL_strcasecmp(shipp->ship_name, "Sathanas") == 0 ) {
...
void ai_cruiser_chase()
{
	...
	// really track down and chase
		...
		// check valid submode
		SDL_assert( (aip->submode == SM_ATTACK) || (aip->submode == SM_BIG_APPROACH) || (aip->submode == SM_BIG_CIRCLE) || (aip->submode == SM_BIG_PARALLEL) );
...
void ai_chase()
{
	...
	if ((real_dot_to_enemy < 0.25f) || (aip->target_time < 1.0f) || (aip->ai_flags & AIF_SEEK_LOCK)) {
		...
		if (aip->targeted_subsys != NULL) {
			SDL_assert(En_objp->type == OBJ_SHIP);
...
*/		}
	...
	SDL_assert( docker_index >= 0 );
	SDL_assert( dockee_index >= 0 );

	SDL_assert(pm0->docking_bays[docker_index].num_slots == 2);
	SDL_assert(pm1->docking_bays[dockee_index].num_slots == 2);
	...
	case DOA_DOCK_STAY:
		...
		if (dock_mode == DOA_DOCK) {
			...
			SDL_assert(dock_mode == DOA_DOCK_STAY);
			objp->orient = dom;
			vector	temp;
...
//		else
	...
	// -- Note, A lot of callers don't care about fdist, so OK to return ERROR value: SDL_assert(fdist != UNINITIALIZED_VALUE);
	return fdist;
...
void remove_farthest_attacker(int objnum)
{
	...
	if (farthest_objp != NULL) {
		ai_info	*aip;
		SDL_assert(farthest_objp->type == OBJ_SHIP);
		SDL_assert((farthest_objp->instance > -1) && (farthest_objp->instance < MAX_SHIPS));
		SDL_assert(Ships[farthest_objp->instance].ai_index > -1);
...
void guard_object_was_hit(object *guard_objp, object *hitter_objp)
{
	...
	SDL_assert( (hitter_objp->type == OBJ_SHIP) || (hitter_objp->type == OBJ_ASTEROID) || (hitter_objp->type == OBJ_WEAPON) );
...
float get_cylinder_points(object *other_objp, object *cyl_objp, vector *axis_pt, vector *r_vec, float *radius)
{
	...
	SDL_assert(cyl_objp->type == OBJ_SHIP);

	// get radius of cylinder
	polymodel *pm = model_get(Ships[cyl_objp->instance].modelnum);
	float tempx, tempy;
	tempx = SDL_max(-pm->mins.xyz.x, pm->maxs.xyz.x);
	tempy = SDL_max(-pm->mins.xyz.y, pm->maxs.xyz.y);
	*radius = SDL_max(tempx, tempy);
...
*/
	...
	//	For now (3/31/97), it's getting trapped by an SDL_assert, so clean it up.
		...
		if (dist_to_goal_point > MAX_GUARD_DIST + 1.5 * (Pl_objp->radius + guard_objp->radius)) {
			...
			if (guard_objp->type == OBJ_SHIP) {
				...
				if (guard_aip->dock_objnum != -1) {
					speed = SDL_max(speed, Objects[guard_aip->dock_objnum].phys_info.speed);
				}
			}
...
object *ai_find_docked_object( object *docker )
{
	...
	SDL_assert( docker->type == OBJ_SHIP );		// this had probably better be a ship!!!
	aip = &Ai_info[Ships[docker->instance].ai_index];
...
void ai_do_objects_repairing_stuff( object *repaired_objp, object *repair_objp, int how )
{
	...
	SDL_assert( repaired_objp->type == OBJ_SHIP);
...
void ai_still()
{
	...
	SDL_assert(Pl_objp->type == OBJ_SHIP);
	SDL_assert((Pl_objp->instance >= 0) && (Pl_objp->instance < MAX_OBJECTS));
...
void ai_dock()
{
	...
	//	float	dist;
		...
		} //else {
		{
			...
			if (aip->path_cur-aip->path_start >= aip->path_length-1) {		//	If got this far, advance no matter what.
				...
				aip->submode_start_time = Missiontime;
				aip->path_cur--;
				SDL_assert(aip->path_cur-aip->path_start >= 0);
	...
	case AIS_DOCK_4: {
		...
		SDL_assert((aip->goal_objnum >= -1) && (aip->goal_objnum < MAX_OBJECTS));
	...
	case AIS_UNDOCK_4: {
		...
		//SDL_assert( !(aip->ai_flags & AIF_DOCKED) );
		//SDL_assert( !(other_aip->ai_flags & AIF_DOCKED) );
		//SDL_assert( !(aip->ai_flags & AIF_REPAIRING) );
		//SDL_assert( !(other_aip->ai_flags & AIF_BEING_REPAIRED) );
		//SDL_assert( !(other_aip->ai_flags & AIF_AWAITING_REPAIR) );
	...
	if (ss->turret_enemy_objnum != -1)	{
		...
		if (ss->targeted_subsys != NULL) {
			...
			if ((lep->type == OBJ_SHIP) && (Ship_info[Ships[lep->instance].ship_info_index].flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP))) {
				ai_big_pick_attack_point_turret(lep, ss, &gun_pos, &gun_vec, &enemy_point, tp->turret_fov, SDL_min(weapon_travel_dist, Weapon_info[tp->turret_weapon_type].weapon_range));
			} else {
				enemy_point = lep->pos;
			}
		}
...
ship_subsys *aifft_find_turret_subsys(object *objp, ship_subsys *ssp, object *enemy_objp, float *dot_out)
{
	...
	SDL_assert(enemy_objp->type == OBJ_SHIP);
	...
	*dot_out = best_dot;
	...
	SDL_assert(best_subsysp != &eshipp->subsys_list);
...
int turret_should_fire_aspect(ship_subsys *turret, float dot, int weapon_class)
{
	...
	if ( (dot > AICODE_TURRET_DUMBFIRE_ANGLE) && (turret->turret_time_enemy_in_range >= SDL_min(wip->min_lock_time,AICODE_TURRET_MAX_TIME_IN_RANGE)) ) {
		return 1;
	}
...
void turret_swarm_fire_from_turret(ship_subsys *turret, int parent_objnum, int target_objnum, ship_subsys *target_subsys)
{
	...
	turret_weapon_class = turret->system_info->turret_weapon_type;
	SDL_assert(Weapon_info[turret_weapon_class].wi_flags & WIF_SWARM);
...
void ai_fire_from_turret(ship *shipp, ship_subsys *ss, int parent_objnum)
{
	...
	SDL_assert( shipp->objnum == parent_objnum );
	...
	weapon_firing_range = SDL_min(Weapon_info[tp->turret_weapon_type].lifetime * Weapon_info[tp->turret_weapon_type].max_speed, Weapon_info[tp->turret_weapon_type].weapon_range);
	...
	if ( turret_should_pick_new_target(ss) ) {
		...
		SDL_assert(objnum < 0 || is_target_beam_valid(ss, objnum));
		...
		if (ss->turret_enemy_objnum != -1) {
			...
			ss->turret_next_enemy_check_stamp = timestamp((int) (SDL_max(dot, 0.5f)*2000.0f) + 1000);
		} else {
	...
	SDL_assert(ss->turret_enemy_objnum != -1);
...
object * get_wing_leader(int wingnum)
{
	...
	SDL_assert(wingp->current_count != 0);			//	Make sure there is a leader
...
void get_absolute_wing_pos(vector *result_pos, object *leader_objp, int wing_index, int formation_object_flag)
{
	...
	wing_spread_size = SDL_max(50.0f, 3.0f * get_wing_largest_radius(leader_objp, formation_object_flag) + 15.0f);
...
int ai_formation()
{
	...
	SDL_assert(Pl_objp->type == OBJ_SHIP);
	SDL_assert((Pl_objp->instance >= 0) && (Pl_objp->instance < MAX_SHIPS));
	...
	SDL_assert((aip->ai_flags & AIF_FORMATION) != AIF_FORMATION);	//	Make sure not both types of formation flying in effect.
	...
	if (aip->ai_flags & AIF_FORMATION_OBJECT) {
		...
		SDL_assert(aip->ai_flags & AIF_FORMATION_WING);
	...
	if (dist_to_goal > 500.0f) {
		...
		if (chaotic_leader) {
			...
			set_accel_for_target_speed(Pl_objp, SDL_min(leader_speed*0.8f, 20.0f));
...
void ai_do_repair_frame(object *objp, ai_info *aip, float frametime)
{
	...
	if (aip->ai_flags & (AIF_BEING_REPAIRED | AIF_AWAITING_REPAIR)) {
		...
		//SDL_assert(repair_aip->mode == AIM_DOCK);

		if (aip->ai_flags & AIF_BEING_REPAIRED) {
			// SDL_assert(repair_aip->submode == AIS_DOCK_4);
...
void ai_chase_circle(object *objp)
{
	...
	SDL_assert(vm_vec_mag(&aip->goal_point) >= 0.0f);		//	Supposedly detects bogus vector
...
*/
	...
	SDL_assert(pm->n_paths > path_index);
...
int ai_find_shockwave_weapon(object *objp, ai_info *aip)
{
	...
	for ( mo = GET_NEXT(&Missile_obj_list); mo != END_OF_LIST(&Missile_obj_list); mo = GET_NEXT(mo) ) {
		...
		wip = &Weapon_info[wp->weapon_info_index];
		SDL_assert( wip->subtype == WP_MISSILE );
...
int aas_1(object *objp, ai_info *aip, vector *safe_pos)
{
	...
	if (aip->ai_flags & AIF_AVOID_SHOCKWAVE_WEAPON) {
		...
		SDL_assert(aip->shockwave_object > -1);
...
//	validate_mode_submode(aip);
	...
	SDL_assert((aip->mode != AIM_WAYPOINTS) || (aip->active_goal != AI_ACTIVE_GOAL_DYNAMIC));
...
//	if (!SDL_strncasecmp(Ships[Pl_objp->instance].ship_name, "cancer", 6))
...
//	}
	...
	SDL_assert(objnum != target_objnum);
...
*/
	...
	if ((En_objp != NULL) && (En_objp->pos.xyz.x == Pl_objp->pos.xyz.x) && (En_objp->pos.xyz.y == Pl_objp->pos.xyz.y) && (En_objp->pos.xyz.z == Pl_objp->pos.xyz.z)) {
		mprintf(("Warning: Object and its enemy have same position.  Object #%li\n", Pl_objp-Objects));
		En_objp = NULL;
	}
...
void ai_set_default_behavior(object *obj, int classnum)
{
	...
	SDL_assert(obj != NULL);
	SDL_assert(obj->instance != -1);
	SDL_assert(Ships[obj->instance].ai_index != -1);
...
void maybe_process_friendly_hit(object *objp_hitter, object *objp_hit, object *objp_weapon)
{
	...
	if ((objp_hitter == Player_obj) && (Player_ship->team == TEAM_FRIENDLY)) {
		...
		SDL_assert(objp_hitter->type == OBJ_SHIP);
		SDL_assert(objp_hit->type == OBJ_SHIP);
		SDL_assert(objp_weapon->type == OBJ_WEAPON);
		...
		float accredited_damage = SDL_min(MAX_BURST_DAMAGE, pp->damage_this_burst + damage) - pp->damage_this_burst;
...
{
	SDL_assert(Ship_info[Ships[aip->shipnum].ship_info_index].flags & (SIF_FIGHTER | SIF_BOMBER));
		...
		if (Objects[parent].signature == other_obj->parent_sig) {
			...
			if (Ships[ship_objp->instance].team != Ships[Objects[parent].instance].team) {
				...
				if (!dont_count) {
					...
					aip->lethality = SDL_min(110.0f, aip->lethality);
					// if you hit, don;t be less than 0
					aip->lethality = SDL_max(0.0f, aip->lethality);
...
void ai_ship_hit(object *objp_ship, object *hit_objp, vector *hitpos, int shield_quadrant, vector *hit_normal)
{
	...
	if (hit_objp->type == OBJ_WEAPON) {
		...
		SDL_assert((hitter_objnum >= 0) && (hitter_objnum < MAX_OBJECTS));
...
void ai_ship_destroy(int shipnum, int method)
{
	...
	for ( so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so) ) {
		...
		SDL_assert(other_objp->instance != -1);
...
int ai_abort_rearm_request(object *requester_objp)
{
	...
	SDL_assert((requester_objp->instance >= 0) && (requester_objp->instance < MAX_SHIPS));	
	...
	if (requester_aip->ai_flags & (AIF_AWAITING_REPAIR | AIF_BEING_REPAIRED)){
		...
		if (requester_aip->dock_objnum != -1) {
			...
			if (repair_objp->signature == requester_aip->dock_signature) {
				...
				SDL_assert( repair_objp->type == OBJ_SHIP );
...
void ai_add_rearm_goal( object *requester_objp, object *support_objp )
{
	...
	SDL_assert( support_shipp->ai_index != -1 );
...
int ai_return_path_num_from_dockbay(object *dockee_objp, int dockbay_index)
{
	...
	if ( dockee_objp->type == OBJ_SHIP ) {
		...
		// sanity checks
		SDL_assert(pm->n_docks > dockbay_index);
		SDL_assert(pm->docking_bays[dockbay_index].num_spline_paths > 0);
		SDL_assert(pm->docking_bays[dockbay_index].splines != NULL);
...
void maybe_cheat_fire_synaptic(object *objp, ai_info *aip)
{
	...
	if ( The_mission.flags & MISSION_FLAG_SUBSPACE )	{
		...
		if (!(SDL_strncasecmp(shipp->ship_name, NOX("delta"), 5))) {
