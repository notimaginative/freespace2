...
static void gles2_stuff_fog_value(float z, float *f_val)
{
	float f_float;
...
static void gles2_tmapper_internal(int nv, vertex **verts, uint flags, int is_scaler)
{
	int i;
	...
	if (flags & TMAP_FLAG_TEXTURED) {
		if ( !gles2_tcache_set(gr_screen.current_bitmap, tmap_type) ) {
	...
	gles2_set_state( texture_source, alpha_blend, zbuffer_type );
	...
	for (i = nv-1; i >= 0; i--) {
		...
		if (bFog) {
			...
			gles2_stuff_fog_value(va->z, &f_val);
...
void gles2_aabitmap_ex_internal(int x, int y, int w, int h, int sx, int sy)
{
	if ( (w < 1) || (h < 1) ) {
		return;
	}
	...
	if ( !gles2_tcache_set(gr_screen.current_bitmap, TCACHE_TYPE_AABITMAP) ) {
		// Couldn't set texture
		mprintf(( "WARNING: Error setting aabitmap texture!\n" ));
		return;
	}

	gles2_set_state( TEXTURE_SOURCE_NO_FILTERING, ALPHA_BLEND_ALPHA_BLEND_ALPHA, ZBUFFER_TYPE_NONE );
...
{
	gles2_rect_internal(x, y, w, h, gr_screen.current_color.red,
...
void gr_gles2_shade(int x, int y, int w, int h)
{
	...
	gles2_rect_internal(x, y, w, h, r, g, b, a);
}
...
void gr_gles2_aabitmap_ex(int x, int y, int w, int h, int sx, int sy)
{
	...
	gles2_aabitmap_ex_internal(x, y, w, h, sx, sy);
}
...
void gr_gles2_aabitmap(int x, int y)
{
	...
	gr_gles2_aabitmap_ex(x, y, w, h, 0, 0);
}
...
void gr_gles2_circle(int xc, int yc, int d)
{
	...
	while(x<y)	{
		...
		gr_gles2_line( xc-y, yc-x, xc+y, yc-x );
		gr_gles2_line( xc-y, yc+x, xc+y, yc+x );
		...
		if (p<0)
			...
			gr_gles2_line( xc-x, yc-y, xc+x, yc-y );
			gr_gles2_line( xc-x, yc+y, xc+x, yc+y );
...
void gr_gles2_flash(int r, int g, int b)
{
	...
	if ( r || g || b ) {
		gles2_set_state(TEXTURE_SOURCE_NONE, ALPHA_BLEND_ALPHA_ADDITIVE, ZBUFFER_TYPE_NONE);
...
{
	gles2_tmapper_internal(nverts, verts, flags, 0);
}
