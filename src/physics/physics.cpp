...
*/
	...
	if ( pi->flags & PF_IN_SHOCKWAVE ) {
		tangles.p += (float) (myrand()-MY_RAND_MAX/2)/MY_RAND_MAX * shock_amplitude;
		tangles.h += (float) (myrand()-MY_RAND_MAX/2)/MY_RAND_MAX * shock_amplitude;
	}
...
void physics_sim(vector* position, matrix* orient, physics_info* pi, float sim_time)
{
	...
	if (pi->flags & PF_CONST_VEL) {
		...
		pi->fspeed = vm_vec_dot(&orient->v.fvec, &pi->vel);		// instead of vector magnitude -- use only forward vector since we are only interested in forward velocity
	}
...
//			Int3();	// Get dave A
		...
		vm_vec_scale_add2( &pi->desired_vel, &orient->v.rvec, pi->prev_ramp_vel.xyz.x );
		vm_vec_scale_add2( &pi->desired_vel, &orient->v.uvec, pi->prev_ramp_vel.xyz.y );
		vm_vec_scale_add2( &pi->desired_vel, &orient->v.fvec, pi->prev_ramp_vel.xyz.z );
...
void physics_apply_shock(vector *direction_vec, float pressure, physics_info *pi, matrix *orient, vector *min, vector *max, float radius)
{
	...
	normal.xyz.x = vm_vec_dotprod( direction_vec, &orient->v.rvec );
	normal.xyz.y = vm_vec_dotprod( direction_vec, &orient->v.uvec );
	normal.xyz.z = vm_vec_dotprod( direction_vec, &orient->v.fvec );
	...
	if ( normal.xyz.x < 0.0f )		// normal < 0, hits the right face
		vm_vec_copy_scale( &impact_vec, &orient->v.rvec, max->xyz.x * pressure * area.xyz.x *  normal.xyz.x * sin.xyz.x / pi->mass );
	else								// normal > 0, hits the left face
		vm_vec_copy_scale( &impact_vec, &orient->v.rvec, min->xyz.x * pressure * area.xyz.x * -normal.xyz.x * sin.xyz.x / pi->mass );
	...
	if ( normal.xyz.y < 0.0f )
		vm_vec_copy_scale( &impact_vec, &orient->v.uvec, max->xyz.y * pressure * area.xyz.y *  normal.xyz.y * sin.xyz.y / pi->mass );
	else
		vm_vec_copy_scale( &impact_vec, &orient->v.uvec, min->xyz.y * pressure * area.xyz.y * -normal.xyz.y * sin.xyz.y / pi->mass );
	...
	if ( normal.xyz.z < 0.0f )
		vm_vec_copy_scale( &impact_vec, &orient->v.fvec, max->xyz.z * pressure * area.xyz.z *  normal.xyz.z * sin.xyz.z / pi->mass );
	else
		vm_vec_copy_scale( &impact_vec, &orient->v.fvec, min->xyz.z * pressure * area.xyz.z * -normal.xyz.z * sin.xyz.z / pi->mass );
