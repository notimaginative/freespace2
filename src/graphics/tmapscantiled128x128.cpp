...
/*
 * $Logfile: /Freespace2/code/Graphics/TmapScanTiled128x128.cpp $
 * $Revision$
 * $Date$
 * $Author$
 *
 * Routines for drawing tiled 128x128 textues
 *
 * $Log$
 * Revision 1.2  2002/06/09 04:41:18  relnev
...
// Make ESI =  DV:DU in 7:9,7:9 format
	mov	eax, Tmap.DeltaV
	shr	eax, 7
	mov	esi, Tmap.DeltaU
	shl	esi, 9
...
// Make ECX = V:U in 7:9,7:9 format
	mov	eax, Tmap.VFixed
	shr	eax, 7
	mov	ecx, Tmap.UFixed
	shl	ecx, 9
...
InnerInnerLoop:
		...
		mov	eax, ecx							// EAX = V.VF:U.UF in 7.9:7.9
		shr	ax, 9								// EAX = V:U in 7.9:16.0
		rol	eax, 7							// EAX = V:U in 0.0:7:7
		and	eax, 03fffh						// clear upper bits
...
Skip0:
		...
		rol	eax, 7							// EAX = V:U in 0.0:7:7
		and	eax, 03fffh						// clear upper bits
		add	eax, Tmap.pixptr		// EAX = (V*128)+U + Pixptr
