...
/*
 * $Logfile: /Freespace2/code/Fred2/WeaponEditorDlg.cpp $
 * $Revision$
 * $Date$
 * $Author$
 *
 ...
 * 
 * 17    5/14/98 5:31p Hoffoss
 * Fixed some stupid behaviour in this editor related to ammo counts being
 * -99.
 * 
 * 16    4/22/98 11:27a Hoffoss
 * Make big weapons available for capital ships as well as big ships (why
 * the hell a capital ship isn't a big ship is beyond me, though).
 * 
 * 15    4/14/98 11:46a Johnson
 * Added window enables for spinners, but it seems to be ignored.
 * 
 * 14    2/26/98 10:44a Johnson
 * Fixed bug with SetRange() not being called on editor first open.
 * 
 * 13    2/22/98 1:32a Hoffoss
 * Changed editor to use raw ammo counts isntead of percentages, and make
 * it select first item in list by default.
 * 
 * 12    2/13/98 3:38p Johnson
 * Fixed bug where multi ship editing won't update player ships.
 * 
 * 11    12/11/97 5:46p Hoffoss
 * Changed Fred to not display weapons that are not available to various
 * ships.
 * 
 * 10    9/16/97 9:41p Hoffoss
 * Changed Fred code around to stop using Parse_player structure for
 * player information, and use actual ships instead.
 * 
 * 9     5/30/97 4:50p Hoffoss
 * Added code to allow marked ship editing of data in child dialogs of
 * ship editor dialog.
 * 
 * 8     4/21/97 5:02p Hoffoss
 * Player/player status editing supported, and both saved and loaded from
 * Mission files.
 * 
 * 7     4/16/97 2:02p Hoffoss
 * Fixed bug Mike created by changing code while I had file checked out
 * still.
 * 
 * 6     4/16/97 1:59p Hoffoss
 * Weapon editor now fully functional.
 * 
 * 5     4/10/97 3:20p Mike
...
void WeaponEditorDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	if (pDX->m_bSaveAndValidate) {
		...
		m_spin1.SetRange(0, static_cast<short>(m_ammo_max1));
		m_spin2.SetRange(0, static_cast<short>(m_ammo_max2));
		m_spin3.SetRange(0, static_cast<short>(m_ammo_max3));
		m_spin4.SetRange(0, static_cast<short>(m_ammo_max4));
