...
void shipfx_subsystem_mabye_create_live_debris(object *ship_objp, ship *ship_p, ship_subsys *subsys, vector *exp_center, float exp_mag)
{
	// initializations
	...
	model_get_rotating_submodel_axis(&model_axis, &world_axis, ship_p->modelnum, submodel_num, ship_objp);
	vm_vec_copy_scale(&rotvel, &world_axis, sii->cur_turn_rate);

	model_find_world_point(&world_axis_pt, &sii->pt_on_axis, ship_p->modelnum, submodel_num, &ship_objp->orient, &ship_objp->pos);
	...
	for (i=0; i<num_live_debris; i++) {
		...
		model_find_world_point(&start_world_pos, &pm->submodel[live_debris_submodel].offset, ship_p->modelnum, live_debris_submodel, &ship_objp->orient, &ship_objp->pos );
		...
		model_find_world_point(&end_world_pos, &start_model_pos, ship_p->modelnum, submodel_num, &ship_objp->orient, &ship_objp->pos);

		// create fireball here.
		fireball_create(&end_world_pos, FIREBALL_EXPLOSION_MEDIUM, OBJ_INDEX(ship_objp), pm->submodel[live_debris_submodel].rad);

		// create debris
		live_debris_obj = debris_create(ship_objp, ship_p->modelnum, live_debris_submodel, &end_world_pos, exp_center, 1, exp_mag);
		...
		if (pm->submodel[pm->debris_objects[idx]].is_live_debris) {
			...
			if ( !pm->submodel[parent].blown_off ) {
				...
				if (pss != NULL) {
					...
					model_find_world_point(&exp_center, &tmp, shipp->modelnum, parent, &ship_objp->orient, &ship_objp->pos );

					// if not blown off, blow it off
					shipfx_subsystem_mabye_create_live_debris(ship_objp, shipp, pss, &exp_center, 3.0f);

					// now set subsystem as blown off, so we only get one copy
					pm->submodel[parent].blown_off = 1;
					set_ship_submodel_as_blown_off(&Ships[ship_objp->instance], pss->system_info->name);
				}
			}
		}
	}
...
void shipfx_blow_off_subsystem(object *ship_objp,ship *ship_p,ship_subsys *subsys, vector *exp_center)
{
	vector subobj_pos;
...
*/
	...
	shipfx_blow_up_model(ship_objp, ship_p->modelnum, psub->subobj_num, 50, &subobj_pos );

	// create live debris objects, if any
	// TODO:  some MULITPLAYER implcations here!!
	shipfx_subsystem_mabye_create_live_debris(ship_objp, ship_p, subsys, exp_center, 1.0f);

	// create first fireball
	fireball_create( &subobj_pos, FIREBALL_EXPLOSION_MEDIUM, OBJ_INDEX(ship_objp), psub->radius );
}
...
static void split_ship_init( ship* shipp, split_ship* split_shipp )
{
	...
	vm_vec_copy_scale(&split_shipp->front_ship.local_pivot, &orient->v.fvec, dist);
	(void) vm_vec_make(&split_shipp->front_ship.model_center_disp_to_orig_center, 0.0f, 0.0f, -dist);
	dist = (split_shipp->back_ship.cur_clip_plane_pt +pm->mins.xyz.z)/2.0f;
	vm_vec_copy_scale(&split_shipp->back_ship.local_pivot, &orient->v.fvec, dist);
	(void) vm_vec_make(&split_shipp->back_ship.model_center_disp_to_orig_center, 0.0f, 0.0f, -dist);
	...
	vm_vec_scale_add2(&split_shipp->front_ship.phys_info.vel, &orient->v.fvec, front_vel);
	vm_vec_scale_add2(&split_shipp->back_ship.phys_info.vel,  &orient->v.fvec, back_vel);
