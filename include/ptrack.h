...
// ------------------------------------------------------------------------------------------------------------
// FREESPACE
// NOTE : this is obsolete for FS2  -  we use our own FS2 struct
#define MAX_FS_MEDALS		16			// these should correspond directly to those defined inside of freespace.
#define MAX_FS_SHIP_TYPES	75			// this one will be scaled down to be the correct # eventually
typedef struct vmt_freespace_struct {
	...
	int kills[MAX_FS_SHIP_TYPES];		 // only valid kills (i.e. not on friendlies).
	int assists;
...
// FREESPACE2
#ifdef FS2_DEMO
	#define MAX_FS2_MEDALS					16						// these should correspond directly to those defined inside of freespace.
	#define MAX_FS2_SHIP_TYPES				120					// this one will be scaled down to be the correct # eventually
	typedef struct vmt_freespace2_struct {
		char				tracker_id[TRACKER_ID_LEN];
		char				pilot_name[PILOT_NAME_LEN];
		...
		ushort			kills[MAX_FS2_SHIP_TYPES];		 // only valid kills (i.e. not on friendlies).
		int				assists;
		int				kill_count;                // total alltime kills
		int				kill_count_ok;			    // total valid alltime kills (no friendlies)
		unsigned int	p_shots_fired;	 // primary weapon
...
#else 
	#define MAX_FS2_MEDALS			NUM_MEDALS			// these should correspond directly to those defined inside of freespace.
	#define MAX_FS2_SHIP_TYPES		MAX_SHIP_TYPES		// this one will be scaled down to be the correct # eventually	
	typedef struct vmt_freespace2_struct {
		char				tracker_id[TRACKER_ID_LEN];
		char				pilot_name[PILOT_NAME_LEN];
		...
		unsigned short num_medals;
		unsigned short num_ship_types;

		int				medals[MAX_FS2_MEDALS];   
		unsigned short kills[MAX_FS2_SHIP_TYPES];		 // only valid kills (i.e. not on friendlies).
	} vmt_freespace2_struct;
#endif
