...
/*
 * $Logfile: /Freespace2/code/Graphics/TmapScanTiled64x64.cpp $
 * $Revision$
 * $Date$
 * $Author$
 *
...
// Make ESI =  DU:DV in 6:10,6:10 format
	mov	eax, Tmap.DeltaV
	shr	eax, 6
	mov	esi, Tmap.DeltaU
	shl	esi, 10
...
// Make ECX = U:V in 6:10,6:10 format
	mov	eax, Tmap.VFixed
	shr	eax, 6
	mov	ecx, Tmap.UFixed
	shl	ecx, 10
...
InnerInnerLoop:
		...
		shr	ax, 10								// EAX = V:U in 6.10:16.0
		rol	eax, 6							// EAX = V:U in 0.0:6:6
		and	eax, 0fffh						// clear upper bits
...
void tmapscan_pln8_tiled_64x64()
{
	if (gr_zbuffering) {
		...
		case GR_ZBUFF_FULL:		// both
			tmapscan_pln8_zbuffered_tiled_64x64();
			return;
