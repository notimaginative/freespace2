 ...
 * Remove an SDL_assert that was easy to trip with time compressed 8x.
 * 
 ...
 * Convert SDL_assert() to nprintf when point of collisions is farther apart
...
int ship_ship_check_collision(collision_info_struct *ship_ship_hit_info, vector *hitpos)
{
	...
	SDL_assert( light_obj->type == OBJ_SHIP );
...
//						submodel_hit = mc.hit_point;
						...
						SDL_assert(is_valid_matrix(&m_temp));
...
int check_special_cruiser_asteroid_collision(object *heavy, object *light, float *cruiser_mass, int *cruiser_light)
{
	...
	if (heavy->type == OBJ_ASTEROID) {
		SDL_assert(light->type == OBJ_SHIP);
		...
		SDL_assert(heavy->type == OBJ_SHIP);
...
//		}
	...
	SDL_assert( !vm_is_vec_nan(&direction_light) );
...
{
	return (SDL_strncasecmp(Ships[objp->instance].ship_name, NOX("planet"), 6) == 0);
}
	...
	if (Ship_info[Ships[big_obj->instance].ship_info_index].flags & (SIF_CAPITAL|SIF_SUPERCAP)) {
		if (Ship_info[Ships[small_obj->instance].ship_info_index].flags & (SIF_SMALL_SHIP)) {
			...
			if (big_speed > 3*big_obj->phys_info.max_vel.xyz.z) {
				...
				vm_vec_scale_add(&perp, &temp, &big_obj->orient.v.fvec, -vm_vec_dotprod(&temp, &big_obj->orient.v.fvec));
...
//			vm_vec_add(&world_hit_pos, &ship_ship_hit_info.heavy->pos, &ship_ship_hit_info.hit_pos);
		...
		if (ship_is_beginning_warpout_speedup(A)) {
			shipA_max_speed = SDL_max(ship_get_max_speed(&Ships[A->instance]), ship_get_warp_speed(A));
		} else {
			shipA_max_speed = ship_get_max_speed(&Ships[A->instance]);
		}

		// Maybe warping in or finished warping in with excessive speed
		shipA_max_speed = SDL_max(shipA_max_speed, vm_vec_mag(&A->phys_info.vel));
		shipA_max_speed = SDL_max(shipA_max_speed, 10.0f);

		// get shipB max speed
		if (ship_is_beginning_warpout_speedup(B)) {
			shipB_max_speed = SDL_max(ship_get_max_speed(&Ships[B->instance]), ship_get_warp_speed(B));
		} else {
			shipB_max_speed = ship_get_max_speed(&Ships[B->instance]);
		}

		// Maybe warping in or finished warping in with excessive speed
		shipB_max_speed = SDL_max(shipB_max_speed, vm_vec_mag(&B->phys_info.vel));
		shipB_max_speed = SDL_max(shipB_max_speed, 10.0f);
...
//	SDL_assert(vm_vec_mag(&r_light) > core_rad - 0.1);
