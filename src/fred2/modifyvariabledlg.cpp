...
void CModifyVariableDlg::OnSelchangeModifyVariableName()
{
	...
	if (m_modified_type || m_modified_name || m_modified_value) {
...
BOOL CModifyVariableDlg::OnInitDialog() 
{
	...
	for (i=0; i<MAX_SEXP_VARIABLES; i++) {
		if (Sexp_variables[i].type & SEXP_VARIABLE_SET) {
			...
			if ( !((box_index == CB_ERR) || (box_index == CB_ERRSPACE)) ) {
				m_traslate_combo_to_sexp[box_index] = i;
			}
		}
	}
	...
	if (m_start_index > -1) {
		for (i=0; i<MAX_SEXP_VARIABLES; i++) {
			if (m_traslate_combo_to_sexp[i] == m_start_index) {
				last_modified = i;
				break;
			}
		}
	...
	if (m_traslate_combo_to_sexp[last_modified] > -1) {
		CEdit *edit = (CEdit *) GetDlgItem(IDC_MODIFY_DEFAULT_VALUE);
		edit->SetWindowText(Sexp_variables[m_traslate_combo_to_sexp[last_modified]].text);
	}

	// Set old variable name
	m_old_var_name = Sexp_variables[m_traslate_combo_to_sexp[last_modified]].variable_name;
	
	// Set type
	m_type_number = (Sexp_variables[last_modified].type & SEXP_VARIABLE_NUMBER);
	set_variable_type();
...
void CModifyVariableDlg::OnOK() 
{
	...
	if (m_data_validated) {
		...
		if (m_var_name_validated) {
			...
			if (m_modified_name || m_modified_value || m_modified_type) {
				m_do_modify = true;
			}
			CDialog::OnOK();
		}
...
void CModifyVariableDlg::validate_var_name(int set_focus)
{
	...
	m_old_var_name = Sexp_variables[m_traslate_combo_to_sexp[cur_sel]].variable_name;
	...
	if (length == 0) {
		...
		if (rval != length) {
			...
			if ( (index != -1) && (index != m_traslate_combo_to_sexp[m_combo_last_modified_index]) ) {
				message = true;
...
{
	int index = m_traslate_combo_to_sexp[m_combo_last_modified_index];
