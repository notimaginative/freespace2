...
int skip_to_string(const char *pstr, const char *end)
{
	...
	while ((*Mp != EOF_CHAR) && SDL_strncasecmp(pstr, Mp, len)) {
		...
		if (end && !SDL_strncasecmp(pstr, Mp, len2))
			return -1;
...
void read_file_text(const char *filename, int mode)
{
	...
	SDL_assert(file_len < MISSION_TEXT_SIZE);
...
void stuff_boolean(int *i)
{
	...
	if ( isdigit(token[0]) ) {					// using 1/0 instead of YES/NO
		...
		if ( !SDL_strcasecmp(token, "yes") ) {
			*i = 1;
		}
		else if ( !SDL_strcasecmp(token, "no") ) {
			*i = 0;
		}
		else
...
int stuff_string_list(char slp[][NAME_LENGTH], int max_strings)
{
	...
	while (*Mp != ')') {
		...
		SDL_assert ( *Mp == '\"' );					// should always be enclosed in quotes
...
//			ilp[num] = 1;
			...
			stuff_int(&tval);
			SDL_assert((tval >= 0) && (tval < max_ints));
			ilp[tval] = 1;
		}
...
int split_str(const char *src, int max_pixel_w, int *n_chars, char **p_str, int max_lines, char ignore_char)
{
	...
	SDL_assert(src != NULL);
	SDL_assert(n_chars != NULL);
	SDL_assert(p_str != NULL);
	SDL_assert(max_lines > 0);
	SDL_assert(max_pixel_w > 0);
