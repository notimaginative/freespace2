 ...
 * main hall.
 * 
 * 85    9/01/99 10:49p Dave
 * Added nice SquadWar checkbox to the client join wait screen.
 * 
 * 84    8/30/99 2:49p Jefff
 * 
 * 83    8/26/99 8:49p Jefff
 * Updated medals screen and about everything that ever touches medals in
 * one way or another.  Sheesh.
 * 
 * 82    8/25/99 4:38p Dave
 * Updated PXO stuff. Make squad war report stuff much more nicely.
 * 
 * 81    8/20/99 2:09p Dave
 * PXO banner cycling.
 * 
 * 80    8/20/99 10:06a Jefff
 * removed closed/rstricted buttons from multi start screen
 * 
 * 79    8/18/99 11:30a Jefff
 * 
 * 78    8/18/99 10:38a Jefff
 * 
 * 77    8/16/99 4:06p Dave
 * Big honking checkin.
 * 
 * 76    8/16/99 1:08p Jefff
 * added sounds to a few controls, made input boxes lose focus on ENTER
 * 
 * 75    8/16/99 9:52a Jefff
 * fixed bitmap loading on buttons in multi-sync screen
 * 
 * 74    8/11/99 5:54p Dave
 * Fixed collision problem. Fixed standalone ghost problem.
 * 
 * 73    8/10/99 4:35p Jefff
 * fixed hi-res coords
 * 
 * 72    8/06/99 12:29a Dave
 * Multiple bug fixes.
 * 
 * 71    8/05/99 3:13p Jasenw
 * tweaked some text placement coords.
 * 
 * 70    8/04/99 1:38p Jefff
 * moved some text in multi join wait
 * 
 * 69    8/03/99 12:45p Dave
 * Update checksums.
 * 
 * 68    7/25/99 5:17p Jefff
 * campaign descriptions show up on multicreate screen
 * 
 * 67    7/20/99 1:49p Dave
 * Peter Drake build. Fixed some release build warnings.
 * 
 * 66    7/19/99 2:13p Dave
 * Added some new strings for Heiko.
 * 
 * 65    7/15/99 9:20a Andsager
 * FS2_DEMO initial checkin
 * 
 * 64    7/08/99 10:53a Dave
 * New multiplayer interpolation scheme. Not 100% done yet, but still
 * better than the old way.
 * 
 * 63    6/30/99 10:49a Jasenw
 * Fixed coords for new launch countdown ani
 * 
 * 62    6/29/99 7:39p Dave
 * Lots of small bug fixes.
 * 
 * 61    6/25/99 11:59a Dave
 * Multi options screen.
 * 
 * 60    6/09/99 2:17p Dave
 * Fixed up pleasewait bitmap rendering.
 * 
 * 59    6/05/99 3:42p Dave
 * New multi sync screen.
 * 
 * 58    6/01/99 6:07p Dave
 * New loading/pause/please wait bar.
 * 
 * 57    5/21/99 6:45p Dave
 ...
 * 
 * 56    5/06/99 11:10a Dave
 * Fixed coord on multi create screen.
 * 
 * 55    5/04/99 6:38p Dave
 * Finished multi join-wait screen.
 * 
 * 54    5/04/99 5:20p Dave
 * Fixed up multiplayer join screen and host options screen. Should both
 * be at 100% now.
 * 
 * 53    5/03/99 11:04p Dave
 * Most of the way done with the multi join screen.
 * 
 * 52    5/03/99 8:32p Dave
 * New version of multi host options screen.
 * 
 * 51    4/29/99 2:15p Neilk
 * slider2 code got modified; changed parameters for create
 * 
 * 50    4/25/99 3:02p Dave
 * Build defines for the E3 build.
 * 
 * 49    4/21/99 6:15p Dave
 ...
 * 
 * 48    4/16/99 5:27p Neilk
 * added slider support and hir res for multi_create
 * 0
 * 47    4/14/99 6:37p Dave
 * Fixed scroll button bug on host create screen.
 * 
 * 46    4/14/99 5:28p Dave
 * Minor bug fixes.
 * 
 * 45    4/12/99 10:07p Dave
 * Made network startup more forgiving. Added checkmarks to dogfight
 * screen for players who hit commit.
 * 
 * 44    4/09/99 2:21p Dave
 * Multiplayer beta stuff. CD checking.
 * 
 * 43    4/08/99 1:28p Dave
 * Small bug fixes for refresh button on the multi create screen.
 * 
 * 42    4/08/99 11:55a Neilk
 * Converted Multi_Create to new artwork (just lowres)
 * 
 * 41    4/08/99 2:10a Dave
 * Numerous bug fixes for the beta. Added builtin mission info for the
 * beta.
 * 
 * 40    3/20/99 3:48p Andsager
 * Do mission_loop stuff for PXO
 * 
 * 39    3/10/99 6:50p Dave
 * Changed the way we buffer packets for all clients. Optimized turret
 * fired packets. Did some weapon firing optimizations.
 * 
 * 38    3/09/99 6:24p Dave
 * More work on object update revamping. Identified several sources of
 * unnecessary bandwidth.
 * 
 * 37    3/08/99 7:03p Dave
 * First run of new object update system. Looks very promising.
 * 
 * 36    2/25/99 4:19p Dave
 ...
 * 
 * 21    1/12/99 4:07a Dave
 * Put in barracks code support for selecting squad logos. Properly
 * distribute squad logos in a multiplayer game.
 * 
 * 20    1/11/99 7:19p Neilk
 * Converted multi_join interface to support multiple resolutions
 * 
 * 19    12/18/98 1:13a Dave
 * Rough 1024x768 support for Direct3D. Proper detection and usage through
 * the launcher.
 * 
 * 18    12/17/98 4:50p Andsager
 ...
 * Need to test now.
 * 
 * 16    12/10/98 10:19a Andsager
 * Fix mission loop assert
 * 
 * 15    12/10/98 9:59a Andsager
 * Fix some bugs with mission loops
 * 
 * 14    12/09/98 1:56p Andsager
 ...
 * 
 * 333   10/02/98 3:22p Allender
 * fix up the -connect option and fix the -port option
 * 
 * 332   9/17/98 9:26p Dave
 * Externalized new string.
 * 
 * 331   9/17/98 3:08p Dave
 ...
 * only 1 player.
 * 
 * 328   9/15/98 7:24p Dave
 * Minor UI changes. Localized bunch of new text.
 * 
 * 327   9/15/98 4:03p Dave
 ...
 * hosts.
 * 
 * 325   9/13/98 9:36p Dave
 * Support for new info icons for multiplayer missions (from-volition,
 * valid, mission disk, etc).
 * 
 * 324   9/11/98 4:14p Dave
...
void multi_common_set_text(const char *str, int auto_scroll)
{
	...
	if(strlen(str) > MULTI_COMMON_MAX_TEXT){
		...
		SDL_strlcpy(Multi_common_all_text, str, sizeof(Multi_common_all_text));
	}
...
void multi_common_add_text(const char *str, int auto_scroll)
{
	...
	if((strlen(str) + strlen(Multi_common_all_text)) > MULTI_COMMON_MAX_TEXT){
		...
		SDL_strlcat(Multi_common_all_text, str, sizeof(Multi_common_all_text));
	}
...
{
	SDL_strlcpy(Multi_common_notify_text, "", sizeof(Multi_common_notify_text));
...
{
	if(str){
		SDL_strlcpy(Multi_common_notify_text, str, sizeof(Multi_common_notify_text));
...
{	
	...
	case MULTI_VOICE_STATUS_DENIED:
		if(Multi_common_icons[MICON_VOICE_DENIED] != -1){
			gr_set_bitmap(Multi_common_icons[MICON_VOICE_DENIED], GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL, 1.0f, -1, -1);
			gr_bitmap(0,0);
		}
		break;
	...
	case MULTI_VOICE_STATUS_RECORDING:
		if(Multi_common_icons[MICON_VOICE_RECORDING] != -1){
			gr_set_bitmap(Multi_common_icons[MICON_VOICE_RECORDING], GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL, 1.0f, -1, -1);
			gr_bitmap(0,0);
		}
		break;
...
DCF(mj_make, "")
{
	...
	for(idx=0; idx<Dc_arg_int; idx++){
		...
		SDL_snprintf(ag.name, sizeof(ag.name), "Game %d", idx);
...
void multi_join_game_do_frame()
{
	...
	if(Multi_join_bitmap != -1){		
		gr_set_bitmap(Multi_join_bitmap, GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL, 1.0f, -1, -1);
		gr_bitmap(0,0);
	}
...
void multi_join_display_games()
{
	...
	if(moveup != NULL){
		do {			
			...
			str[0] = '\0';
			SDL_strlcpy(str, Multi_join_speed_labels[con_type], sizeof(str));
			...
			if(moveup->flags & AG_FLAG_STANDALONE){
				SDL_strlcat(str, MJ_CHAR_STANDALONE, sizeof(str));
			}
			if(moveup->flags & AG_FLAG_CAMPAIGN){
				SDL_strlcat(str, MJ_CHAR_CAMPAIGN, sizeof(str));
			}

			// tack on the actual server name			
			SDL_strlcat(str, " ", sizeof(str));
			SDL_strlcat(str, moveup->name, sizeof(str));
			if(strlen(moveup->mission_name) > 0){
				SDL_strlcat(str, " / ", sizeof(str));
				SDL_strlcat(str, moveup->mission_name, sizeof(str));
			} 
			...
			if(moveup->ping.ping_avg > 0){
				if(moveup->ping.ping_avg > 1000){
					gr_set_color_fast(&Color_bright_red);
					SDL_strlcpy(str, XSTR("> 1 sec",761), sizeof(str));
				} else {
					...
					SDL_snprintf(str, sizeof(str), "%d%s", moveup->ping.ping_avg, XSTR(" ms",762));
				}
			...
			SDL_snprintf(str, sizeof(str), "%d", moveup->num_players);
...
void multi_join_blit_game_status(active_game *game, int y)
{
	...
	case AG_FLAG_COOP:
		if(Multi_common_icons[MICON_COOP] != -1){
			gr_set_bitmap(Multi_common_icons[MICON_COOP], GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL, 1.0f, -1, -1);
			draw = 1;
		}
		break;	
	...
	case AG_FLAG_TEAMS:
		if(Multi_common_icons[MICON_TVT] != -1){
			gr_set_bitmap(Multi_common_icons[MICON_TVT], GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL, 1.0f, -1, -1);
			draw = 1;
		} 
		break;	
	...
	case AG_FLAG_DOGFIGHT:
		if(Multi_common_icons[MICON_DOGFIGHT] != -1){
			gr_set_bitmap(Multi_common_icons[MICON_DOGFIGHT], GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL, 1.0f, -1, -1);
			draw = 1;
		} 
		break;	
#endif
	...
	case AG_FLAG_FORMING:
		gr_set_color_fast(&Color_bright_green);
		SDL_strlcpy(status_text, XSTR("Forming", 764), sizeof(status_text));
		break;
	case AG_FLAG_BRIEFING:
		gr_set_color_fast(&Color_bright_red);
		SDL_strlcpy(status_text, XSTR("Briefing", 765), sizeof(status_text));
		break;
	case AG_FLAG_DEBRIEF:
		gr_set_color_fast(&Color_bright_red);
		SDL_strlcpy(status_text, XSTR("Debrief", 766), sizeof(status_text));
		break;
	case AG_FLAG_PAUSE:
		gr_set_color_fast(&Color_bright_red);
		SDL_strlcpy(status_text, XSTR("Paused", 767), sizeof(status_text));
		break;
	case AG_FLAG_IN_MISSION:
		gr_set_color_fast(&Color_bright_red);
		SDL_strlcpy(status_text, XSTR("Playing", 768), sizeof(status_text));
		break;
	default:
		gr_set_color_fast(&Color_bright);
		SDL_strlcpy(status_text, XSTR("Unknown", 769), sizeof(status_text));
		break;
	}		
...
void multi_join_load_tcp_addrs()
{
	...
	while(!cfeof(file)){
		...
		if ( !psnet_is_valid_ip_string(line) ) {
			...
			addr.type = NET_TCP;
			psnet_string_to_addr(&addr, line, sizeof(line));
			if ( addr.port == 0 ){
...
{	
	...
	if(Multi_join_selected_item->flags & AG_FLAG_PASSWD){
		if(!multi_passwd_popup(Multi_join_request.passwd, sizeof(Multi_join_request.passwd))){
			return;
		}
	...
	SDL_strlcpy(Multi_join_request.callsign, Player->callsign, sizeof(Multi_join_request.callsign));
	if(strlen(Player->image_filename) > 0){
		SDL_strlcpy(Multi_join_request.image_filename, Player->image_filename, sizeof(Multi_join_request.image_filename));
	}	
#ifndef MAKE_FS1
	if(strlen(Player->squad_filename) > 0){
		SDL_strlcpy(Multi_join_request.squad_filename, Player->squad_filename, sizeof(Multi_join_request.squad_filename));
	}
#endif
	...
	// pxo squad info
#ifndef MAKE_FS1
	SDL_strlcpy(Multi_join_request.pxo_squad_name, Multi_tracker_squad_name, LOGIN_LEN);
#endif
...
{	
	...
	if(Multi_has_cd && (Multi_common_icons[MICON_CD] != -1)){
		...
		gr_set_bitmap(Multi_common_icons[MICON_CD], GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL, 1.0f, -1, -1);
...
void multi_start_game_init()
{
	...
	if ( Cmdline_start_netgame ) {
		if ( Cmdline_game_name != NULL ) {
			SDL_strlcpy( Multi_sg_netgame->name, Cmdline_game_name, sizeof(Multi_sg_netgame->name) );
			Multi_sg_game_name.set_text(Multi_sg_netgame->name);
		}
		...
		if ( Cmdline_closed_game ) {
			...
			SDL_strlcpy(Multi_sg_netgame->passwd, Cmdline_game_password, sizeof(Multi_sg_netgame->passwd));
...
void multi_start_game_do()
{
	...
	if(Multi_sg_bitmap != -1){
		gr_set_bitmap(Multi_sg_bitmap, GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL, 1.0f, -1, -1);
		gr_bitmap(0,0);
	}
...
void multi_sg_init_gamenet()
{
	...
	memset(buf,0,128);
	pilot_format_callsign_personal(Player->callsign, out_name, sizeof(out_name));
	SDL_snprintf(buf, sizeof(buf), XSTR("%s game",782), out_name);  // [[ %s will be a pilot's name ]]
	if ( strlen(buf) > MAX_GAMENAME_LEN ){
		SDL_strlcpy(buf, XSTR("Temporary name",783), sizeof(buf));
	}
	SDL_strlcpy(Multi_sg_netgame->name, buf, sizeof(Multi_sg_netgame->name));
...
void multi_sg_rank_display_stuff()
{
	...
	while((count < NUM_RANKS) && (count < Multi_sg_rank_max_display[gr_screen.res]) && (idx < NUM_RANKS)){	
		...
		// print the text
		multi_sg_rank_build_name(Ranks[idx].name, rank_name, sizeof(rank_name));
	...
	if(Multi_sg_rank_button.pressed()){		 
		...
		if(item + Multi_sg_rank_start < NUM_RANKS){		
			...
			if(multi_sg_rank_select_valid(item + Multi_sg_rank_start)){
				...
				memset(string,0,255);
				SDL_snprintf(string,sizeof(string),XSTR("Illegal value for a host of your rank (%s)\n",784),Ranks[Net_player->player->stats.rank].name);
...
void multi_sg_rank_build_name(char *in, char *out, const int max_outlen)
{
	...
	if (SDL_strcasecmp(first,XSTR("lieutenant",785)) == 0) {
		...
		if(first != NULL){
			SDL_snprintf(out, max_outlen, "%s%s", XSTR("Lt. ",786), first); // [[ lieutenant ]]
		}
...
void multi_create_select_to_filename(int select_index, char *filename, const int max_filelen);
	...
	if ( !Multi_create_files_loaded ) {
		...
		if ( MULTIPLAYER_CLIENT ) {
			...
			if(Multi_create_bitmap != -1){
				gr_set_bitmap(Multi_create_bitmap, GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL, 1.0f, -1, -1);
				gr_bitmap(0, 0);
			}
			chatbox_render();
			if ( loading_bitmap > -1 ){
				gr_set_bitmap(loading_bitmap, GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL, 1.0f, -1, -1);
			}
...
void multi_create_plist_blit_normal()
{
	...
	for(idx=0;idx<MAX_PLAYERS;idx++){		
		...
		if(MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx])){
			...
			SDL_strlcpy(str, Net_players[idx].player->callsign, sizeof(str));
			if(Net_players[idx].flags & NETINFO_FLAG_OBSERVER){
				SDL_strlcat(str, XSTR("(O)",787), sizeof(str));  // [[ Observer ]]
			}
...
void multi_create_plist_blit_team()
{
	...
	for(idx=0;idx<MAX_PLAYERS;idx++){
		...
		if(MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx]) && (Net_players[idx].p_info.team == 0)){
			...
			SDL_strlcpy(str, Net_players[idx].player->callsign, sizeof(str));
			if(Net_players[idx].flags & NETINFO_FLAG_OBSERVER){
				SDL_strlcat(str, XSTR("(O)",787), sizeof(str));
			}
			gr_force_fit_string(str,CALLSIGN_LEN,Mc_players_coords[gr_screen.res][MC_W_COORD] - total_offset);
	...
	for(idx = 0; idx < file_count; idx++){
		...
		if ( flags ) {
			...
			if ( Multi_create_mission_count < MULTI_CREATE_MAX_LIST_ITEMS ) {
				...
				SDL_strlcpy(mcip->filename, filename, sizeof(mcip->filename));
				SDL_strlcpy(mcip->name, mission_name, sizeof(mcip->name));
				mcip->flags = flags;
...
{	
	...
	for(idx = 0; idx < file_count; idx++){
		...
		flags = mission_campaign_parse_is_multi( filename, name, sizeof(name) );
		if( flags != CAMPAIGN_TYPE_SINGLE && mission_campaign_get_info(filename,title,&campaign_type,&max_players)) {
			if ( Multi_create_campaign_count < MULTI_CREATE_MAX_LIST_ITEMS ) {
				...
				SDL_strlcpy(mcip->filename, filename, sizeof(mcip->filename));
				SDL_strlcpy(mcip->name, name, sizeof(mcip->name));
...
void multi_create_list_do()
{
	...
	for(idx=start_index; idx<stop_index; idx++){
		...
		multi_create_select_to_filename(Multi_create_list_select, selected_name, sizeof(selected_name));
		...
		SDL_strlcpy(selected_name, Multi_create_file_list[idx].name, sizeof(selected_name));
		...
		SDL_strlcpy(selected_name, Multi_create_file_list[idx].filename, sizeof(selected_name));
...
void multi_create_list_select_item(int n)
{
	...
	if ( n != Multi_create_list_select ) {
		...
		if(Multi_create_list_mode == MULTI_CREATE_SHOW_MISSIONS){
			multi_create_select_to_filename(n, ng->mission_name, sizeof(ng->mission_name));
		} else {
			multi_create_select_to_filename(n, ng->campaign_name, sizeof(ng->campaign_name));
		}
		...
		case MULTI_CREATE_SHOW_MISSIONS:		
			...
			if(Net_player->flags & NETINFO_FLAG_AM_MASTER){			
				...
				SDL_strlcpy(ng->title, The_mission.name, sizeof(ng->title));
		...
		case MULTI_CREATE_SHOW_CAMPAIGNS:
			...
			if(Net_player->flags & NETINFO_FLAG_AM_MASTER){
				...
				// if we successfully got the # of players
					...
					SDL_strlcpy(ng->title, title, sizeof(ng->title));
...
void multi_create_list_blit_icons(int list_index, int y_start)
{
	...
	if((fb != NULL) && (fb->flags & FSB_FROM_VOLITION)){
		if(Multi_common_icons[MICON_VOLITION] >= 0){
			gr_set_bitmap(Multi_common_icons[MICON_VOLITION], GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL, 1.0f, -1, -1);
...
void multi_create_accept_hit()
{
	...
	case MULTI_CREATE_SHOW_MISSIONS:	
		if(Multi_create_list_select != -1){
			...
			multi_create_select_to_filename(Multi_create_list_select, selected_name, sizeof(selected_name));
			SDL_strlcpy( Game_current_mission_filename, selected_name, MAX_FILENAME_LEN );
			SDL_strlcpy( Netgame.mission_name, selected_name, MAX_FILENAME_LEN );
...
void multi_create_select_to_filename(int select_index, char *filename, const int max_filelen)
{
	...
	if(Multi_create_list_mode == MULTI_CREATE_SHOW_MISSIONS){
		for(idx=0;idx<Multi_create_mission_count;idx++){
			...
			if(select_index < 0){
				SDL_strlcpy(filename, Multi_create_file_list[idx].filename, max_filelen);
				return;
			}
		}
	}
	...
	SDL_strlcpy(filename, "", max_filelen);
}
...
int multi_create_ok_to_commit()
{
	...
	for(idx=0; idx<MAX_PLAYERS; idx++){
		...
		if(MULTI_CONNECTED(Net_players[idx]) && (Net_players[idx].flags & NETINFO_FLAG_HAXOR)){
			...
			if(Net_players[idx].player != NULL){
				SDL_snprintf(err_string, sizeof(err_string), "%s %s", Net_players[idx].player->callsign, XSTR("has hacked tables/data", 1271));
			} else {
				SDL_snprintf(err_string, sizeof(err_string), "somebody %s", XSTR("has hacked tables/data", 1271));
			}
	...
	if(observer_count > Netgame.options.max_observers){
		...
		SDL_snprintf(err_string,sizeof(err_string),XSTR("There are too many observers in the game\n\nMax : %d\nCurrently %d\n\nPlease dump a few",791),Netgame.options.max_observers,observer_count);
	...
	if(player_count > Netgame.max_players){
		// print up the error string
		SDL_snprintf(err_string,sizeof(err_string),XSTR("There are too many players in the game\n\nMax : %d\nCurrently %d\n\nPlease dump a few", 792), Netgame.max_players,player_count);
...
void multi_host_options_do()
{
	...
	if(Multi_ho_bitmap != -1){
		gr_set_bitmap(Multi_ho_bitmap, GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL, 1.0f, -1, -1);
		gr_bitmap(0,0);
	}
...
{	
	...
	SDL_snprintf(resp_str,sizeof(resp_str),"%d",Netgame.options.max_observers);
	Multi_ho_obs.set_text(resp_str);

	// set the respawn count
	if(Netgame.campaign_mode == MP_SINGLE){
		memset(resp_str,0,10);
		SDL_snprintf(resp_str,sizeof(resp_str),"%d",Netgame.respawn);
	...
	SDL_snprintf(resp_str,sizeof(resp_str),"%d",(int)(tl / 60.0f));
	Multi_ho_time_limit.set_text(resp_str);

	// set the furball kill limit
	memset(resp_str,0,10);
	SDL_snprintf(resp_str,sizeof(resp_str),"%d",Netgame.options.kill_limit);
	Multi_ho_kill_limit.set_text(resp_str);

	// set the token wait time
	memset(resp_str,0,10);
	SDL_snprintf(resp_str,sizeof(resp_str),"%d",Netgame.options.voice_token_wait / 1000);
...
void multi_ho_blit_max_respawns()
{
	...
	SDL_snprintf(string,sizeof(string),"(%d)",Multi_ho_mission_respawn);
	gr_set_color_fast(&Color_normal);
...
void multi_game_client_setup_do_frame()
{
	...
	if(Multi_jw_bitmap != -1){		
		gr_set_bitmap(Multi_jw_bitmap, GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL, 1.0f, -1, -1);
		gr_bitmap(0,0);
	}
	...
	if(Netgame.campaign_mode){
		if(strlen(Netgame.campaign_name) > 0){			
			SDL_strlcpy(mission_text, Netgame.campaign_name, sizeof(mission_text));
			
			if(strlen(Netgame.title) > 0){
				SDL_strlcat(mission_text, ", ", sizeof(mission_text));
				SDL_strlcat(mission_text, Netgame.title, sizeof(mission_text));
			}
		...
		if(strlen(Netgame.mission_name) > 0){			
			SDL_strlcpy(mission_text, Netgame.mission_name, sizeof(mission_text));
...
void multi_sync_do()
{
	...
	case MULTI_SYNC_INGAME:
		...
		if(Multi_sync_bitmap != -1){
			gr_set_bitmap(Multi_sync_bitmap, GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL, 1.0f, -1, -1);
			gr_bitmap(0,0);
		}
				...
				if(Net_player->flags & NETINFO_FLAG_AM_MASTER){
					if(Net_players[idx].s_info.xfer_handle != -1){					
						...
						if((pct_complete >= 0.0) && (pct_complete <= 1.0)){						
							SDL_snprintf(txt,sizeof(txt),XSTR("Mission file xfer %d%%",828),(int)(pct_complete * 100.0f));
						}
						// otherwise
						else {
							SDL_strlcpy(txt, XSTR("Mission file xfer", 829), sizeof(txt));
						}					
					} else {
						SDL_strlcpy(txt, XSTR("Mission file xfer", 829), sizeof(txt));
					}
...
void multi_sync_display_name(const char *name, int index, int np_index)
{
	...
	SDL_strlcpy(fit, name, sizeof(fit));
...
void multi_sync_display_status(const char *status, int index)
{
	...
	// make sure the string actually fits
	SDL_strlcpy(fit, status, sizeof(fit));
...
void multi_maybe_set_mission_loop()
{
	...
	if ( (Campaign.missions[cur].has_mission_loop && (Campaign.loop_mission != -1)) && !require_repeat_mission ) {
		...
		char buffer[512];
		debrief_assemble_optional_mission_popup_text(buffer, sizeof(buffer), Campaign.missions[cur].mission_loop_desc);
...
void multi_passwd_do(char *passwd, const int max_passlen)
{
	int k;
	...
	while(Multi_passwd_done == -1){	
		...
		if(Multi_pwd_passwd.changed()){
			SDL_strlcpy(passwd, "", max_passlen);
		...
		gr_set_bitmap(Multi_pwd_bitmap, GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL, 1.0f, -1, -1);
		gr_bitmap(0,0);
...
int multi_passwd_popup(char *passwd, const int max_plen)
{
	...
	// run the popup
	multi_passwd_do(passwd, max_plen);
